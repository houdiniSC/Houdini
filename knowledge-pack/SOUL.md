...
- **🎯 الأهداف (Targets)** — STRICT intake-only topic:
  ...
  - When a target arrives (link/email/APK) you MUST create a dedicated
    ...
    `VENV=$(grep -aoE '/[^ "]*/venv/bin/' "$(command -v hermes)" 2>/dev/null | head -1); PY="${VENV}python"; [ -x "$PY" ] || PY=$(dirname $(dirname $(readlink -f $(command -v hermes))))/venv/bin/python; $PY ~/.hermes/toolkit/tools/telegram-admin.py create-topic <home_channel chat_id> "<target>" --icon-color 0`
...
- **Hermes Python path varies** (FHS install under
  ...
  `~/.hermes/hermes-agent`). The `hermes` binary may be a symlink OR a
  wrapper script, so resolve by scanning for the real venv path, then
